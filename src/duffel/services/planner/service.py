...
from .cache import build_execution_and_cache_insights
from .llm import save_llm_debug_output
from .template_service import get_or_create_itinerary_template
...
class TravelPlannerService(BaseService):
    ...
    def generate_itinerary(
        ...
        with StepLogger.step(1, 6, "Prompt Intent Extraction & Route Classification", f"Query: {prompt[:35]}..."):
            ...
            resolved_origin = origin or intent.get("origin_city") or intent.get("origin") or user_location or "Atlanta"
            user_location = user_location or resolved_origin
            ...
            theme_style = intent.get("theme") or intent.get("style") or style or "balanced"

        with StepLogger.step(2, 6, "Itinerary Template Retrieval (PostgreSQL First)", f"Dest: {dest_clean}, Days: {duration_days}, Style: {theme_style}"):
            template_modules, llm_meta, tmpl_source = get_or_create_itinerary_template(
                config=cfg, dest_clean=dest_clean, origin_code=origin_code, duration_days=duration_days,
                trip_type="road_trip" if is_road_trip else ("cruise" if is_cruise else "flight"),
                style=theme_style, budget=budget or "moderate", passengers_count=passengers_count,
                prompt=prompt, base_lat=base_lat, base_lng=base_lng,
                ...
                is_test_mode=is_test_mode, force_refresh=force_refresh,
            )

        with StepLogger.step(3, 6, "Live Component Pricing & Availability", f"Origin: {origin_code}, Dest: {dest_upper}"):
        ...
        with StepLogger.step(4, 6, "Dynamic Timeline Assembly & Date Binding", f"Dates: {start_date} to {end_date}"):
            assembled_days = ItineraryAssemblyEngine.assemble_itinerary_from_modules(
                modules=template_modules, destination=dest_clean, origin=origin_code,
                start_date=start_date, duration_days=duration_days,
                trip_type="road_trip" if is_road_trip else "flight",
                passengers_count=passengers_count, include_flights=include_flights,
                ...
                return_dep=return_dep, return_arr=return_arr,
                ...
                rooms_calculated=rooms_calculated, cars_calculated=cars_calculated,
                is_road_trip=is_road_trip,
            )
            src_type = "modular_postgres_assembly" if tmpl_source == "modular_postgres_template" else "live_llm"
            llm_days = assembled_days
        ...
        with StepLogger.step(6, 6, "Package Bundles Assembly & Output Export", f"Destination: {dest_clean}"):
            ...
            if self.cache and self.cache.enabled:
                c_key = f"{dest_clean.lower()}-{duration_days}day-{theme_style.lower()}-itinerary"
                self.cache.set(c_key, response_payload, ttl_seconds=3600 * 24)
