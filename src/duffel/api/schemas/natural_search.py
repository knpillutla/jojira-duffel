...
class NaturalSearchRequest(BaseModel):
    ...
    from_date: Optional[str] = Field(None, description="Explicit start date YYYY-MM-DD")
    to_date: Optional[str] = Field(None, description="Explicit end date YYYY-MM-DD")
    min_price: Optional[float] = Field(None, ge=0, description="Minimum price constraint")
...
class NaturalSearchResponse(BaseModel):
    ...
    lowest_non_stop_offers: Optional[list[dict[str, Any]]] = Field(None, description="Up to 10 lowest non-stop flight offers sorted by price ascending")
